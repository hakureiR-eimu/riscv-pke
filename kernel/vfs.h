...
struct vinode_ops {
    ...
    // operations (such as allocating additional data structures) at some
    // critical times.
...
#define viop_disk_stat( node, istat )                                          \
    ( node->i_ops->viop_disk_stat( node, istat ) )
...
#define viop_mkdir( dir, sub_dentry )                                          \
    ( dir->i_ops->viop_mkdir( dir, sub_dentry ) )
#define viop_write_back_vinode( node )                                         \
    ( node->i_ops->viop_write_back_vinode( node ) )
