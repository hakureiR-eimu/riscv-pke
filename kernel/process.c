...
process *alloc_process() {
    ...
    user_vm_map( (pagetable_t) procs[ i ].pagetable,
                 (uint64) procs[ i ].trapframe, PGSIZE,
                 (uint64) procs[ i ].trapframe,
                 prot_to_type( PROT_WRITE | PROT_READ, 0 ) );
    procs[ i ].mapped_info[ CONTEXT_SEGMENT ].va =
        (uint64) procs[ i ].trapframe;
    ...
    user_vm_map( (pagetable_t) procs[ i ].pagetable, (uint64) trap_sec_start,
    ...
    sprint( "in alloc_proc. user frame 0x%lx, user stack 0x%lx, user kstack "
            "0x%lx \n",
            procs[ i ].trapframe, procs[ i ].trapframe->regs.sp,
            procs[ i ].kstack );
    ...
    for ( int i = 0; i < parent->total_mapped_region; i++ ) {
        ...
        case HEAP_SEGMENT:
            ...
            // in the heap when mapping the heap blocks
            {
                ...
                for ( int i = 0; i < parent->user_heap.free_pages_count; i++ ) {
                    int index = ( parent->user_heap.free_pages_address[ i ] -
                                  heap_bottom ) /
                                PGSIZE;
                ...
                for ( uint64 heap_block = current->user_heap.heap_bottom;
                    ...
                    memcpy( child_pa,
                            ...
                            PGSIZE );
                    user_vm_map( (pagetable_t) child->pagetable, heap_block,
        ...
        case CODE_SEGMENT:
            ...
            user_vm_map( (pagetable_t) child->pagetable,
                         parent->mapped_info[ CODE_SEGMENT ].va, PGSIZE,
